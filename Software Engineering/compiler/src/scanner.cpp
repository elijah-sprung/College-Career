...
void scanner::scan_digit()
{
	...
	if (is_int)
	{
		float digit_value = stof(digit);

		if (digit_value < -2147483648 || digit_value > 2147483647)
		{
			error->flag(current_line_number, current_pos_on_line, 68);
			return;
		}
		else
		{
			current_integer_value = digit_value;
	...
	else
	{
		if (exponent.length() > 0)
		{
			...
			if (exponent_value < -126 || exponent_value > 127)
			{
...
{
	if (error->get_recovery())
	{
		while ((current_token->get_sym() != s) && (current_token->get_sym() != symbol::end_of_program))
		{
			get_token();
		}
