...
// Global variable to keep track of the compares made during the quick sort algorithm. If this counter is passed by reference
// like all of the other counters named "compares", then a segmentation fault occurs for some reason that has not been found.
long long int quick_compares = 0;
...
void quickSort(int low, int high, int myarray[]);
void partition(int low, int high, int& pivotpoint, int myarray[]);
...
/*
  fillRandom: Function that will fill an array with random numbers from 1 to 100,000 for manipulation.
...
void quickSort(int low, int high, int myarray[])
{
  ...
  {
    partition(low, high, pivotpoint, myarray);
    quickSort(low, pivotpoint - 1, myarray);
    quickSort(pivotpoint + 1, high, myarray);
  }
}
...
void testDataset(int original[], int sort[])
{
  ...
  cout << setfill('-') << setw(82) << "-" << setfill(' ') << endl;
  cout << setw(15) << "Quick" << setw(5) << "|" << setw(15) << quick_time.count() << setw(5) << "|" << setw(17) << quick_compares << setw(5) << "|" << setw(12) << "(n/a)" << endl;
  cout << setfill('-') << setw(82) << "-" << setfill(' ') << endl;
